...
client.on('message', message => {
    if(message.content.startsWith(prefix + "toggleAntihack")) {
        ...
        if(!antihack[message.guild.id]) antihack[message.guild.id] = {
          ...
          if(antihack[message.guild.id].onoff === 'Off') return [message.channel.send(`**鉁?The AntiHack Is __饾悗饾悕__ !**`), antihack[message.guild.id].onoff = 'On']
          if(antihack[message.guild.id].onoff === 'On') return [message.channel.send(`**鉀?The AntiHack Is __饾悗饾悈饾悈__ !**`), antihack[message.guild.id].onoff = 'Off']
...
message.channel.send(`**鉁?The AntiBots Is __饾悗饾悕__ !**`)
...
message.channel.send(`**鉀?The AntiBots Is __饾悗饾悈饾悈__ !**`)
          fs.writeFile("./antibots.json", JSON.stringify(antibots), (err) => {
            if (err) console.error(err)
            .catch(err => {
              console.error(err);
...
client.user.setGame(`TDK`,"http://twitch.tv/S-F")
...
////// 2- -Kyle,#4434
////// 3- osama,tech.#0184
/////亚蓉 禹演?怯倾?https://discord.gg/QMeKMyg
///// 4- Azoqz#0888




client.login("NTg2NTY2NjY3MTg2ODY0MTc2.XPp9Aw.PAmfdgVn3Afe_p6-hU-IUK_uTOo");
